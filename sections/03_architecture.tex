...
\addlinespace
Fixed, never backend-selected & Crypto, CBOR & n/a & n/a & No Kconfig \texttt{choice}, no backend struct; \texttt{PSA\_CRYPTO}/\texttt{ZCBOR}/\texttt{ZCBOR\_CANONICAL} are \texttt{select}ed unconditionally at the top-level \texttt{menuconfig FIDO2}. \\
\bottomrule
\end{tabular}
\end{table*}

\texttt{fido2\_clientpin.c} does not appear in Table~\ref{tab:fourtier}; it is not a distinct concern with its own backend-selection story, and its relationship to \texttt{fido2\_core.c} is addressed in Section~\ref{subsec:arch-ignorance}.
...
A second, independent artifact confirms the same fixed/pluggable line. The four backend-selectable concerns, \texttt{fido2.h}, \texttt{fido2\_attestation.h}, \texttt{fido2\_transport.h}, \texttt{fido2\_storage.h}, and \texttt{fido2\_up.h}, live under \texttt{include/zephyr/authentication/fido2/}, use the Zephyr public-API include-guard convention (\texttt{ZEPHYR\_INCLUDE\_AUTHENTICATION\_FIDO2\_..\_H\_}), and carry \texttt{@ingroup fido2}, \texttt{@since}, and \texttt{@version} Doxygen tags. Crypto and CBOR use plain local guards (\texttt{FIDO2\_CRYPTO\_H\_}, \texttt{FIDO2\_CBOR\_H\_}) with no \texttt{@ingroup} and no version tags, marking them as internal implementation headers split into separate files for maintainability rather than for pluggability. Two independent artifacts, documentation tooling convention and build configuration, converge on the same fixed/pluggable line without either being derived from the other.

\subsection{Crypto as a Fixed, Delegated Tier}
\label{subsec:arch-crypto}

\texttt{fido2\_crypto.c} is a fixed wrapper, not a backend selected via a struct, and has no Kconfig choice at all. Portability is delegated one level down to the PSA Crypto API~\cite{psa-crypto-api}, which abstracts hardware-backed versus software crypto beneath the subsystem. This is portability-via-delegation, distinct from and weaker than the portability-via-backend-selection that Storage, Attestation, User Presence, and Transport get through Kconfig.

\subsection{Minimal Public Surface}
\label{subsec:arch-surface}

The entire application-facing API is six functions and one callback type: \texttt{fido2\_init()}, \texttt{fido2\_start()}, \texttt{fido2\_stop()}, \texttt{fido2\_set\_state\_callback()}, \texttt{fido2\_get\_state()}, and \texttt{fido2\_reset()}. Nothing else crosses the host/subsystem boundary; a host application that only ever calls these six symbols has no way to observe or alter subsystem-internal state.
...
\texttt{fido2\_clientpin.c} is not a peer module subject to the rule above; it is an extension of \texttt{fido2\_core.c}, factored into a separate file to keep \texttt{fido2\_core.c} from growing unwieldy, not a boundary-isolated concern in its own right. It has no public header, no Kconfig entry, and no backend-selection mechanism, unlike every module in Table~\ref{tab:fourtier}'s pluggable tiers. Consequently its direct includes of \texttt{fido2\_crypto.h}, \texttt{fido2\_cbor.h}, and \texttt{fido2\_storage.h} are part of core's own orchestration logic, not a violation of module mutual ignorance. \texttt{fido2\_core.c} and \texttt{fido2\_clientpin.c} together are the only files permitted to include more than one of the module headers above.
